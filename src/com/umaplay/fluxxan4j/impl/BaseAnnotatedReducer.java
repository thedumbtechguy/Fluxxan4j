...
import com.umaplay.fluxxan4j.annotation.BindAction;
...
public abstract class BaseAnnotatedReducer<State> extends BaseReducer<State> {
    ...
    protected final ConcurrentHashMap<String, Method> mActionMap = new ConcurrentHashMap<String, Method>();
    ...
    public DispatchResult<State> reduce(State state, Action action) throws Exception {
        if(mActionMap.containsKey(action.Type)) {
            ...
            return new DispatchResult<State>((State) method.invoke(this, state, action.Payload), true);
        }

        return new DispatchResult<State>(state, false);
    }
